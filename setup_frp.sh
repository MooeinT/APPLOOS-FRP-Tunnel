...
# ==================================================================================
#
#   APPLOOS FRP TUNNEL - Full Management Script (v45.0 - Auto Nginx for WSS)
...
setup_iran_server() {
    ...
    if [ "$FRP_PROTOCOL" == "wss" ]; then
        echo -e "${YELLOW}--> WSS mode: Installing Nginx and Certbot...${NC}"
        apt-get update -y > /dev/null && apt-get install nginx certbot python3-certbot-nginx -y > /dev/null
        ...
        # Make a backup of the current Nginx config
        cp /etc/nginx/sites-available/default /etc/nginx/sites-available/default.bak > /dev/null 2>&1
        # Stop Nginx temporarily for certbot standalone challenge if needed
        systemctl stop nginx
        certbot certonly --standalone --agree-tos --non-interactive --email you@example.com -d ${FRP_DOMAIN}
        if [ $? -ne 0 ]; then echo -e "${RED}Failed to obtain SSL certificate. Check that your domain correctly points to this server's IP (${IRAN_SERVER_IP}).${NC}"; systemctl start nginx; return 1; fi
...
uninstall_frp() {
    ...
    if [ -f "/etc/nginx/sites-available/default.bak" ]; then mv /etc/nginx/sites-available/default.bak /etc/nginx/sites-available/default; systemctl restart nginx > /dev/null 2>&1; echo -e "${YELLOW}--> Restored old Nginx config.${NC}"; fi
    echo -e "${YELLOW}Note: Firewall rules must be removed manually for safety.${NC}"; echo -e "\n${GREEN}SUCCESS! FRP has been uninstalled.${NC}"
}
main_menu() {
    while true; do
