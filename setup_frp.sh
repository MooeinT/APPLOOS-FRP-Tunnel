...
# ==================================================================================
#
#   APPLOOS FRP TUNNEL - Full Management Script (v30.0 - Final WSS Fix)
...
get_protocol_choice() {
    echo -e "\n${CYAN}Select transport protocol for the main tunnel connection:${NC}\n  1. TCP (Standard)\n  2. WSS (Max Stealth, Requires Domain & Auto-installs Nginx)"
    read -p "Enter choice [1-2] (Default is 1): " proto_choice
    [[ "$proto_choice" == "2" ]] && FRP_PROTOCOL="wss" || FRP_PROTOCOL="tcp"
...
download_and_extract() {
    ...
    echo -e "${GREEN}--> FRP version $(${FRP_INSTALL_DIR}/frps --version) downloaded.${NC}"
}
setup_iran_server() {
    ...
    if [ "$FRP_PROTOCOL" == "wss" ]; then
        ...
        if [ $? -ne 0 ]; then echo -e "${RED}Failed to obtain SSL certificate. Check DNS record for ${FRP_DOMAIN}.${NC}"; systemctl start nginx; return 1; fi
...
EOF
    ...
    if [[ "$FRP_PROTOCOL" == "wss" ]]; then ufw allow 80/tcp > /dev/null; ufw allow 443/tcp > /dev/null; else ufw allow ${FRP_TCP_CONTROL_PORT}/tcp > /dev/null; fi
...
uninstall_frp() {
    ...
    if [ -d "/etc/nginx" ]; then echo -e "${YELLOW}Nginx is not removed automatically.${NC}"; fi
...
main_menu() {
    while true; do
        ...
        echo "================================================="; echo -e "      ${CYAN}APPLOOS FRP TUNNEL${NC} - v30.0"; echo "================================================="
