...
authentication_method = token # Old syntax for reference (may not work on all FRP versions)
token = ${FRP_TOKEN} # Old syntax for reference (may not work on all FRP versions)
# New syntax for FRP >= 0.50.0
auth_token = ${FRP_TOKEN}
auth_method = token # Correct new syntax
EOF
