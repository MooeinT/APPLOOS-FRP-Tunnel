...
# ==================================================================================
#
#   APPLOOS FRP TUNNEL - Full Management Script (v27.0 - Auto Nginx for WSS)
...
get_port_input() {
    read -p $'\n'"Enter TCP & UDP port(s) to tunnel (e.g., 8080, 20000-30000): " user_ports
...
setup_iran_server() {
    ...
    # WSS requires a different setup with Nginx
    if [ "$FRP_PROTOCOL" == "wss" ]; then
        ...
        certbot --nginx --agree-tos --redirect --hsts --staple-ocsp --email temp@temp.com -d ${FRP_DOMAIN}
        if [ $? -ne 0 ]; then echo -e "${RED}Failed to obtain SSL certificate.${NC}"; return 1; fi
...
server {
    ...
    location / {
        proxy_redirect off;
        proxy_pass http://127.0.0.1:7000; # Pass to frps bind_port
...
EOF
        ...
        # frps listens on a local TCP port for Nginx
        cat > ${FRP_INSTALL_DIR}/frps.ini << EOF
[common]
bind_port = 7000
dashboard_addr = 127.0.0.1 # Secure dashboard
...
main_menu() {
    while true; do
        ...
        echo "================================================="; echo -e "      ${CYAN}APPLOOS FRP TUNNEL${NC} - v27.0"; echo "================================================="
