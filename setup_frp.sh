...
# ==================================================================================
#
#   APPLOOS FRP TUNNEL - Full Management Script (v29.0 - Final Config Fixes)
...
setup_iran_server() {
    ...
    if [ "$FRP_PROTOCOL" == "wss" ]; then
        echo -e "${YELLOW}--> WSS mode selected. Installing Nginx and Certbot...${NC}"
        apt-get update > /dev/null && apt-get install nginx certbot python3-certbot-nginx -y > /dev/null
        ...
        certbot --nginx --agree-tos --redirect --hsts --staple-ocsp --non-interactive --email you@example.com -d ${FRP_DOMAIN}
        if [ $? -ne 0 ]; then echo -e "${RED}Failed to obtain SSL certificate. Check your DNS records and logs.${NC}"; return 1; fi
...
# Dashboard is bound to localhost for security when using Nginx
...
tcp_mux = ${TCP_MUX}
EOF
        if [[ "$FRP_PROTOCOL" == "tcp" ]]; then echo "bind_port = ${FRP_TCP_CONTROL_PORT}" >> ${FRP_INSTALL_DIR}/frps.ini; fi
    ...
    # Allow dashboard access only from localhost in WSS mode for security
    if [[ "$FRP_PROTOCOL" != "wss" ]]; then ufw allow ${FRP_DASHBOARD_PORT}/tcp > /dev/null; fi
...
tcp_mux = ${TCP_MUX}
EOF
    case $FRP_PROTOCOL in "tcp") echo "server_port = ${FRP_TCP_CONTROL_PORT}" >> ${FRP_INSTALL_DIR}/frpc.ini ;; "quic") echo "server_port = ${FRP_QUIC_CONTROL_PORT}" >> ${FRP_INSTALL_DIR}/frpc.ini; echo "protocol = quic" >> ${FRP_INSTALL_DIR}/frpc.ini ;; "wss") echo "server_port = 443" >> ${FRP_INSTALL_DIR}/frpc.ini; echo "protocol = wss" >> ${FRP_INSTALL_DIR}/frpc.ini; echo "tls_enable = true" >> ${FRP_INSTALL_DIR}/frpc.ini; echo "server_name = ${FRP_DOMAIN}" >> ${FRP_INSTALL_DIR}/frpc.ini ;; esac
    cat >> ${FRP_INSTALL_DIR}/frpc.ini << EOF
...
uninstall_frp() {
    ...
    if [ -d "/etc/nginx" ]; then echo -e "${YELLOW}--> Nginx configs are not removed automatically.${NC}"; fi
    echo -e "${YELLOW}--> Firewall rules must be removed manually.${NC}"
    echo -e "\n${GREEN}SUCCESS! FRP has been uninstalled.${NC}"
}
main_menu() {
    while true; do
        clear; CURRENT_SERVER_IP=$(wget -qO- 'https://api.ipify.org' || echo "N/A")
        echo "================================================="; echo -e "      ${CYAN}APPLOOS FRP TUNNEL${NC} - v29.0"; echo "================================================="
